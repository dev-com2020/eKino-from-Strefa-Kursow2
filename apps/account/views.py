...
class SiteSignupView(CreateView):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            url = 'http://127.0.0.1:8000' + reverse('account:confirm_email', kwargs={'user_id': user_id,
                                                                                     'token': token})
            message = get_template('account/acc_active_email.html').render({'confirm_url': url})
            mail = EmailMessage('EKINO - potwierdzenie rejestracji', message, to=[user.email],
